...
export const generateInvoicePDF = async (item) => {
  try {
    ...
    const html = `
      ...
      <body>
        <div class="container">
          <h1>Marketbook&solution Invoice</h1>
          ...
          <hr/>
          <p><strong>Bill To:</strong> ${item.customerName || 'Customer'}</p>
          ...
          <p class="status-${item.status}"><strong>Status:</strong> ${item.status.charAt(0).toUpperCase() + item.status.slice(1)}</p>
          <div class="footer">
    ...
    return new Promise((resolve, reject) => {
      pdf.create(html, options).toBuffer((err, buffer) => {
        if (err) {
          ...
          return reject(new Error('Failed to generate invoice PDF'));
        }
...
// using Go to https://pdfshift.io/dashboard and verify invoice generating:
